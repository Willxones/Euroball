...
export default function LeaguePicker() {
  ...
  // Set selected league after data is fetched
  if (data && !selected) {
    setSelected(data.leagueCollection.items[0] || null);
  }
  ...
  return (
    <Listbox value={selected} onChange={setSelected}>
      <div className="relative my-2">
        <ListboxButton className="relative w-full cursor-pointer rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-md dark:bg-gray-700 dark:text-white sm:text-sm sm:leading-6">
          <span className="flex items-center">
            {selected && <img alt="" src={selected.logo.url} className="size-5 shrink-0 object-cover" />}
          ...
          {leagues.map((league) => (
            ...
            >
              <div className="flex items-center">
                <img alt="" src={league.logo.url} className="size-5 shrink-0 object-cover" />
