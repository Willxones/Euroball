...
export default function NewsTable({
  ...
  const { data, loading, error } = useQuery<GetArticlesResponse>(
    selectedLeague?.name === "All Leagues"
      ? GET_ALL_ARTICLES
      : GET_ARTICLES_BY_LEAGUE,
    {
      variables: {
        limit,
        skip,
        leagueName:
          selectedLeague?.name === "All Leagues" ? null : selectedLeague?.name,
  ...
  if (isSidebar && currentArticleId !== undefined) {
    filteredArticles = filteredArticles.filter(
      (article) => article.sys.id !== currentArticleId,
    );
  }

  return (
    <div
      className={`my-2 flex gap-5 ${isSidebar ? "mt-0 flex-col" : "flex-col sm:grid sm:grid-cols-2 sm:flex-row lg:grid-cols-3"}`}
    >
