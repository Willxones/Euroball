...
import { GET_ARTICLE_BY_ID, GET_LEAGUE_BY_ID, GET_WEEK_BY_ID } from "../../queries";
...
export default function ScoresModal({ isModalOpen, selectedGame, homeTeam, awayTeam, onCloseModal }: ScoresModalProps) {
  const weekId = selectedGame?.week?.sys.id;

  // Fetch week data only if weekId is defined and valid
  const { data: weekData, loading: weekLoading, error: weekError } = useQuery(GET_WEEK_BY_ID, {
    variables: { weekId },
  ...
  const { data: leagueData, loading: leagueLoading, error: leagueError } = useQuery(GET_LEAGUE_BY_ID, {
    variables: { leagueId },
    skip: !leagueId, // Skip if no league ID
  });

  // Fetch article data based on relatedArticle ID from selectedGame
  const { data: articleData, loading: articleLoading, error: articleError } = useQuery<{ article: Article }>(
    GET_ARTICLE_BY_ID,
    {
      variables: {
  ...
  const replaySection = () => {
    if (selectedGame?.gameReplay) {
      ...
      return <div><p className="text-center">This game has no highlights or replay</p></div>;
    }
  };
  ...
  return (
    <>
      <Modal show={isModalOpen} size="lg" onClose={onCloseModal} dismissible={true}>
        <Modal.Header>{selectedGame ? formatDateTime(selectedGame.dateAndTime) : 'Game Details'}</Modal.Header>
        <Modal.Body>
          <div className="space-y-4 dark:text-white">
            ...
            <div className="flex">
              <div className={`flex flex-1 gap-2 ${isHomeScoreHighest ? 'font-bold' : ''}`}>
