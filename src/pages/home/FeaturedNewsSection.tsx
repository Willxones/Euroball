...
export default function FeaturedNewsSection() {
  const {
    ...
    error: featuredError,
  } = useQuery(GET_FEATURED_ARTICLES);
  const {
    data: allData,
    loading: allLoading,
    error: allError,
  ...
  if (combinedArticles.length < 3) {
    const nonDuplicateRecentArticles = recentArticles.filter(
      (article: Article) =>
        !combinedArticles.some(
          (featured) => featured.sys.id === article.sys.id,
        ),
    );

    combinedArticles.push(
      ...nonDuplicateRecentArticles.slice(0, 3 - combinedArticles.length),
    );
  }
    ...
    <>
      ...
      <div className="my-2 flex w-full flex-col gap-2 overflow-hidden dark:text-white md:h-96 md:flex-row lg:h-[400px]">
        ...
        <div className="flex w-full flex-col gap-2 md:h-40 lg:h-full">
          {combinedArticles[1] && (
            <FeaturedSideArticle article={combinedArticles[1]} />
          )}
          {combinedArticles[2] && (
            <FeaturedSideArticle article={combinedArticles[2]} />
          )}
        </div>
      </div>
    </>
